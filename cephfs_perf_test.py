...
class CephFSPerfTest:
    def __init__(self, config_path, inventory_path):
        ...
        cluster_json_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "cluster.json"
        )
        ...
        if not self.mdss:
            print(
                "Warning: No 'mdss' group found in inventory, falling back to 'osds' for MDS placement."
            )
    ...
    def unmount_path_pattern(self, host_name, pattern):
        ...
        for mnt in mount_points:
            ...
            self.run_remote(
                host_name, f"sudo umount -f {mnt} || sudo umount -l {mnt} || true"
            )
            ...
            while time.time() - start_wait < 60:
                ...
                for fsys in filesystems:
                    if fsys.get("mdsmap", {}).get("fs_name") == fs:
                        ...
                        if (
                            up
                        ):  # if up is not empty, it means there are still active/assigned MDS
                            found_active = True
                        break
                if not found_active:
                    break
    ...
    def provision_ganesha(self, use_custom_config=True, results_dir=None):
        ...
        for idx, fs in enumerate(self.fs_names):
            ...
            subprocess.run(
                [
                    ...
                    port,
                    local_export_file,
                    f"{user}@{host}:{remote_export_path}",
                ]
            )
        ...
        for g_host in self.ganeshas:
            ...
            if not asok_path or "No such file or directory" in asok_path:
                print(
                    f"[{g_host}] Warning: Ganesha admin socket not found for 'config diff'."
                )
                continue

            print(f"[{g_host}] Running 'config diff' via {asok_path}...")
            diff_output = self.run_remote(
    ...
    def cleanup_ganesha(self):
        ...
        exports_raw = self.run_remote(
            self.admin, f"sudo ceph nfs export ls {svc_id} --format json"
        )
        ...
        if exports:
            ...
            for exp in exports:
                ...
                self.run_remote(
                    self.admin, f"sudo ceph nfs export rm {svc_id} {export_path}"
                )
        else:
            ...
            for fs in self.fs_names:
                ...
                self.run_remote(
                    self.admin,
                    f"sudo ceph nfs export rm {svc_id} {export_path} || true",
                )
    ...
    def generate_mds_yaml(self, fs, count, current_settings=None):
        ...
        if current_settings and "cpus" in current_settings:
            mds_spec["extra_container_args"].extend(
                ["--cpus", str(current_settings["cpus"])]
            )
        ...
        if self.config["mds_yaml_path"] != local_mds_yaml:
            ...
            subprocess.run(
                [
                    ...
                    port,
                    local_mds_yaml,
                    f"{user}@{host}:{self.config['mds_yaml_path']}",
                ]
            )

    def generate_ganesha_yaml(self, svc_id, selected_hosts, use_custom_config=False):
        print(
            f"Generating ganesha.yaml for {svc_id} (custom_config={use_custom_config})..."
        )
        ...
        for fs in self.fs_names:
            for key, value in settings.items():
                if key == "max_mds":
                    ...
                    self.run_remote(
                        self.admin, f"sudo ceph config set mds {key} {value}"
                    )
        ...
        for fs in self.fs_names:
            for client_name in self.clients:
                for mnt_idx in range(mounts_per_fs):
                    ...
                    self.run_remote(
                        client_name, f"sudo chown {user}:{user} {mount_path}"
                    )
    ...
    def get_results_dir(self, settings, shared_timestamp=None):
        ...
        fs_part = (
            f"{self.fs_name}-x{self.num_filesystems}-c{num_clients}-m{mounts_per_fs}"
        )
        ...
        for line in process.stdout:
            ...
            if "Starting RUN phase" in line:
                ...
                if (
                    ...
                    self.reset_lockstat()
                if (
                    self.config.get("logging", {}).get("enabled", False)
                    and not logging_triggered_for_current_lp
                ):
                    print(
                        f"Triggering MDS logging for Load Point {current_loadpoint}..."
                    )
                ...
                if ganesha_perf_enabled:
                    print(
                        f"Resetting Ganesha perf counters for Load Point {current_loadpoint}..."
                    )
                    for g_host in self.ganeshas:
            ...
            if "Tests finished" in line:
                ...
                if ganesha_perf_enabled and results_dir:
                    print(
                        f"Collecting Ganesha perf dumps for Load Point {current_loadpoint}..."
                    )
                    ...
                    for g_host in self.ganeshas:
                        ...
                        if dump:
                            ...
                            self.save_json_to_results(
                                g_host, dump, filename, results_dir
                            )
            ...
            if perf_record_enabled:
                if run_phase_started and not perf_triggered_for_current_lp:
                    if "Run " in line and " percent complete" in line:
                        print(
                            f"Triggering perf record for Load Point {current_loadpoint}..."
                        )
                        results_dir = payload.get("results_dir")
                        t = threading.Thread(
    ...
    def execute_perf_record(self, loadpoint, results_dir=None, settings=None):
        ...
        flamegraph_path = self.config["specstorage"].get(
        ...
        def collect_output(server_name, p):
            ...
            if p.returncode != 0:
                ...
                print(
                    f"[{server_name}] Finished perf record for Load Point {loadpoint}."
                )
            ...
            for server_name in self.mdss:
                ...
                if reports_output and "No such file or directory" not in reports_output:
                    ...
                    for report_file in report_files:
                        ...
                        can_read = self.run_remote(
                            server_name, f"test -r {report_file} && echo OK || echo NO"
                        ).strip()
                        if can_read == "OK":
                            ...
                            print(
                                f"[{server_name}] Permission denied for {report_file}, retrying via {tmp_path}..."
                            )
                            self.run_remote(
                                server_name, f"sudo -n cp {report_file} {tmp_path}"
                            )
                            self.run_remote(
                                server_name, f"sudo -n chown {user}:{user} {tmp_path}"
                            )
                            self.run_remote(
                                server_name, f"sudo -n chmod 0644 {tmp_path}"
                            )
        ...
        for server_name in self.mdss:
            ...
            if results_dir:
                ...
                for daemon in daemons:
                    ...
                    # Transfer to admin
                    admin_user, admin_host, admin_port = self.get_ssh_details(
                        self.admin
                    )
    ...
    def start_lockstat(self, fs):
        ...
        for server_name in self.mdss:
            ...
            if self.lockstat_exists[server_name]:
                ...
                print(
                    f"[{server_name}] lockstat.py not found at {lockstat_path}, skipping start"
                )
