...
class WorkloadRunner(abc.ABC):
    ...
    def execute_perf_record(
        ...
        fg_path = workload_cfg.get(
            "perf_record_flamegraph_path", "/cephfs_perf/FlameGraph"
        )
        ...
        options_str = ""
        if settings:
            full_base = CommonUtils.get_workload_base_name(
                workload_name,
                "perf_record",
                "server",
                loadpoint,
        ...
        for server_name in target_nodes:
            ...
            print(
                f"[{server_name}] Executing perf record for Load Point {loadpoint}: {subprocess.list2cmdline(ssh_cmd)}"
            )
        ...
        if results_dir:
            print(
                f"Copying perf reports and stap traces to {results_dir} on {self.admin}..."
            )
